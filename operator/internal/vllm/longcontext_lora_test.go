...
func TestResolveLongContextLoraAndRenderArgs(t *testing.T) {
    ...
    if err := ValidateEffectiveConfig(e); err != nil { t.Fatal(err) }
    args := buildArgs(e); got := strings.Join(args, " ")
    want := "--enable-lora --max-lora-rank 16 --lora-modules adapter=/models/adapters/lora"
    if !strings.Contains(got, want) { t.Fatalf("args %q missing exact composed LoRA args %q", got, want) }
}
...
func TestResolveLongContextRejectsInvalidLoraModules(t *testing.T) {
    ...
    if err != nil { t.Fatal(err) }
    if err := ValidateEffectiveConfig(e); err == nil || !strings.Contains(err.Error(), "invalid loraModules") { t.Fatalf("got %v", err) }
    for _, a := range buildArgs(e) { if a == "--lora-modules" { t.Fatal("invalid config rendered --lora-modules") } }
}
